...
import mongomock
from mongomock import OperationFailure, command_cursor, filtering, helpers
...
class _Parser(object):
    ...
    def _handle_arithmetic_operator(self, operator, values):
        ...
        if operator in binary_arithmetic_operators:
            ...
            if len(values) != 2:
                raise OperationFailure(
                    "%s must have only 2 parameters" % operator
                )
    ...
    def _handle_string_operator(self, operator, values):
        ...
        if operator == "$concat":
            ...
            return (
                ...
                else "".join([str(x) for x in parsed_list])
            )
        if operator == "$split":
            if len(values) != 2:
            ...
            if not isinstance(delimiter, str):
                raise TypeError(
                    "split second argument must evaluate to string"
                )
        ...
        if operator == "$regexMatch":
            ...
            if unknown_args:
                raise OperationFailure(
                    "$regexMatch found an unknown argument: %s"
                    % list(unknown_args)[0]
                )
            ...
            if isinstance(regex_val, str):
                ...
                regex = re.compile(
                    regex_val.pattern, regex_val.flags or options
                )
            else:
                raise OperationFailure(
    ...
    def _handle_date_operator(self, operator, values):
        ...
        if operator == "$dateToString":
            ...
            if not isinstance(values, dict) or not {"format", "date"} <= set(
                values
            ):
                raise OperationFailure(
                    "$dateToString operator must correspond a dict"
    ...
    def _handle_array_operator(self, operator, value):
        ...
        if operator == "$map":
            ...
            for k in ("input", "in"):
                if k not in value:
                    raise OperationFailure(
                        "Missing '%s' parameter to $map" % k
                    )

            for k in value:
                if k not in {"input", "as", "in"}:
                    raise OperationFailure(
                        "Unrecognized parameter to $map: %s" % k
                    )
        ...
        if operator == "$filter":
            ...
            if extra_params:
                raise OperationFailure(
                    "Unrecognized parameter to $filter: %s"
                    % extra_params.pop()
                )
        ...
        if operator == "$slice":
            if not isinstance(value, list):
                raise OperationFailure(
                    "$slice only supports a list as its argument"
                )
            ...
            if len(value) > 2 and value[2] <= 0:
                raise OperationFailure(
                    "Third argument to $slice must be "
    ...
    def _handle_type_convertion_operator(self, operator, values):
        ...
        if operator == "$arrayToObject":
            ...
            if all(
                isinstance(x, dict) and set(x.keys()) == {"k", "v"}
                for x in parsed
            ):
                return {d["k"]: d["v"] for d in parsed}

            if all(
                isinstance(x, (list, tuple)) and len(x) == 2 for x in parsed
            ):
                return dict(parsed)
...
def _parse_expression(
    expression, doc_dict, ignore_missing_keys=False, user_vars=None
):
    """Parse an expression.
    ...
    """
    return _Parser(
        doc_dict, user_vars=user_vars, ignore_missing_keys=ignore_missing_keys
    ).parse(expression)
...
def _accumulate_group(output_fields, group_list, user_vars=None):
    doc_dict = {}
    for field, value in output_fields.items():
        ...
        for operator, key in value.items():
            ...
            for doc in group_list:
                try:
                    values.append(
                        _parse_expression(key, doc, user_vars=user_vars)
                    )
                except KeyError:
                    continue
            if operator in _GROUPING_OPERATOR_MAP:
...
def _handle_lookup_stage(in_collection, database, options, user_vars=None):
        ...
        if pipeline:
            if let:
                ...
                for var, expr in six.iteritems(let):
                    doc_user_vars[var] = _parse_expression(
                        expr, doc, user_vars=user_vars
                    )
            else:
...
def _handle_graph_lookup_stage(
    in_collection, database, options, user_vars=None
):
...
def _handle_group_stage(
    in_collection, unused_database, options, user_vars=None
):
    grouped_collection = []
    _id = options["_id"]
    if _id:
        ...
        def _key_getter(doc):
            try:
                return _parse_expression(
                    _id, doc, ignore_missing_keys=True, user_vars=user_vars
                )
            except KeyError:
                return None
...
def _handle_bucket_stage(
    ...
    for (unused_key, doc_id), group in grouped:
        ...
        doc_dict = _accumulate_group(
            output_fields, group_list, user_vars=user_vars
        )
        doc_dict["_id"] = doc_id
...
def _handle_sample_stage(
    in_collection, unused_database, options, unused_user_vars=None
):
    ...
    for doc in in_collection:
        ...
        for index, field_item in iter_array:
            ...
            if include_array_index:
                new_doc = helpers.set_value_by_dot(
...
def _handle_replace_root_stage(
    in_collection, unused_database, options, user_vars=None
):
...
def _handle_project_stage(
    ...
    projection_spec = _combine_projection_spec(
        filter_list, original_filter=options
    )
    out_collection = [
        _project_by_spec(
            doc, projection_spec, is_include=(method == "include")
        )
        for doc in in_collection
    ]
...
def _handle_add_fields_stage(
    in_collection, unused_database, options, user_vars
):
    if not options:
        raise OperationFailure(
...
def _handle_out_stage(in_collection, database, options, unused_user_vars=None):
...
def _handle_count_stage(
    in_collection, database, options, unused_user_vars=None
):
...
def _handle_facet_stage(in_collection, database, options, user_vars=None):
...
def _handle_match_stage(in_collection, database, options, user_vars=None):
