...
class _Filterer(object):
        ...
        for key, search in search_filter.items():
            ...
            if key in _TOP_LEVEL_OPERATORS:
                raise NotImplementedError(
                    "The {} operator is not implemented in mongomock yet".format(
                        key
                    )
                )
            if key.startswith("$"):
            ...
            if search == {"$exists": False} and not iter_key_candidates(
                key, document
            ):
                continue
            ...
            if isinstance(search, dict) and "$all" in search:
                if not self._all_op(
                    iter_key_candidates(key, document), search["$all"]
                ):
                    return False
            ...
            for doc_val in iter_key_candidates(key, document):
                ...
                if is_ops_filter:
                    ...
                    unknown_operators = (
                        set(search) - set(self._operator_map) - {"$not"}
                    )
                    ...
                    is_match = (
                        all(
                            ...
                            and self._operator_map[operator_string](
                                doc_val, search_val
                            )
                ...
                elif isinstance(search, _RE_TYPES) and isinstance(
                    doc_val, (str, list)
                ):
                    ...
                    is_match = LOGICAL_OPERATOR_MAP[key](
                        document, search, self.apply
                    )
    ...
    def _not_op(self, d, k, s):
        if isinstance(s, dict):
            for key in s.keys():
                if (
                    key not in self._operator_map
                    and key not in LOGICAL_OPERATOR_MAP
                ):
    ...
    def _all_op(self, doc_val, search_val):
        if (
            isinstance(doc_val, list)
            and doc_val
            and isinstance(doc_val[0], list)
        ):
        ...
        for sub_doc in doc:
            if isinstance(sub_doc, dict):
                if sub_key in sub_doc:
                    ret.extend(
                        iter_key_candidates(key_remainder, sub_doc[sub_key])
                    )
                else:
                    ret.append(NOTHING)
        return ret
...
LOGICAL_OPERATOR_MAP = {
    ...
    "$nor": lambda d, subq, filter_func: all(
        not filter_func(q, d) for q in subq
    ),
