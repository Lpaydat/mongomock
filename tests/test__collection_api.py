...
class CollectionAPITest(TestCase):
    ...
    def test__get_subcollections_by_attribute_underscore(self):
        ...
        self.assertIn(
            "Collection has no attribute '_b'", str(err_context.exception)
        )
    ...
    def test__get_collection_read_concern_option(self):
        ...
        self.assertTrue(
            self.db.get_collection("new_collection", read_concern=None)
        )
    ...
    def test__distinct_filter_field(self):
        ...
        self.assertEqual(
            set(self.db.collection.distinct("f1", {"k1": "v1"})),
            set(["v1", "v2"]),
        )
    ...
    def test__insert_many_write_errors(self):
        ...
        self.assertEqual(
            {"nInserted", "writeErrors"}, set(error_details.keys())
        )
    ...
    def test__count_documents(self):
        ...
        self.assertEqual(
            1, self.db.collection.count_documents({}, skip=1, limit=1)
        )
    ...
    def test__find_and_modify_no_projection_kwarg(self):
        with self.assertRaises(
            TypeError
        ):  # unlike pymongo, we warn about this
            self.db.collection.find_and_modify(
                {"a": 2}, {"a": 3}, projection=["a"]
            )
    ...
    def test__regex_options(self):
        ...
        self.assertFalse(
            self.db.collection.find_one({"a": {"$regex": "tada"}})
        )
        self.assertTrue(
        ...
        self.assertTrue(
            self.db.collection.find_one(
                {"a": {"$regex": "tada", "$options": "i"}}
            )
        )
        self.assertTrue(
            self.db.collection.find_one(
                {"a": {"$regex": "^da", "$options": "im"}}
            )
        )
        self.assertFalse(
            self.db.collection.find_one(
    ...
    def test__update_one_upsert_dots(self):
        ...
        self.assert_document_stored(
            update_result.upserted_id, {"a": {"b": 1}, "c": 2}
        )
    ...
    def test__update_one_unset_position(self):
        insert_result = self.db.collection.insert_one(
            {"a": 1, "b": [{"c": 2, "d": 3}]}
        )
        update_result = self.db.collection.update_one(
    ...
    def test__update_many_upsert(self):
        ...
        self.assertIsNotNone(update_result.upserted_id)
        self.assert_document_stored(
            update_result.upserted_id, {"a": 1, "c": 0}
        )
    ...
    def test__update_push_slice_from_the_end(self):
        ...
        self.assertEqual(
            [50, 60, 80, 78, 86], self.db.collection.find_one()["scores"]
        )
    ...
    def test__update_push_slice_from_the_front(self):
        ...
        self.assertEqual(
            [89, 90, 100], self.db.collection.find_one()["scores"]
        )
    ...
    def test__update_push_slice_only(self):
        ...
        self.assertEqual(
            [70, 100, 20], self.db.collection.find_one()["scores"]
        )

    def test__update_push_slice_nested_field(self):
        self.db.collection.insert_one(
            {"games": [{"scores": [89, 70, 100, 20]}]}
        )
        self.db.collection.update_one(
            {},
            {
    ...
    def test__update_push_slice_positional_nested_field(self):
        ...
        self.assertEqual(
            [0, 1], self.db.collection.find_one()["games"][0]["scores"]
        )
        self.assertEqual(
            [3, 15], self.db.collection.find_one()["games"][1]["scores"]
        )
    ...
    def test__update_push_sort_document(self):
        ...
        self.assertEqual(
            {"b": [5, 4, 3, 2, 1]}, self.db.collection.find_one()["a"]
        )
    ...
    def test__update_push_positional_nested_field(self):
        ...
        self.assertEqual(
            [{"player": {"scores": [15]}}],
            self.db.collection.find_one()["games"],
        )
    ...
    def test__update_push_array_of_arrays(self):
        ...
        self.assertEqual(
            [[0], [1, 15]], self.db.collection.find_one()["games"]
        )
    ...
    def test__cursor_count_with_skip(self):
        ...
        self.db["coll_name"].insert_many([first, second, third])
        count = (
            self.db["coll_name"].find().skip(1).count(with_limit_and_skip=True)
        )
        self.assertEqual(count, 2)
    ...
    def test__find_with_skip_param(self):
        ...
        count = sum(
            1 for d in self.db["users"].find(sort=[("name", 1)], skip=1)
        )
        self.assertEqual(1, count)
        self.assertEqual(
            self.db["users"].find(sort=[("name", 1)], skip=1)[0]["name"],
            "second",
        )
    ...
    def test__ttl_applied_to_compound_key_is_ignored(self):
        ...
        self.db.collection.insert_one(
            {"field1": datetime.utcnow(), "field2": "val2"}
        )
        self.assertEqual(self.db.collection.count_documents({}), 1)
    ...
    def test__ensure_partial_filter_expression_unique_index(self):
        ...
        self.db.collection.create_index(
            ...
            partialFilterExpression={
                "partialFilterExpression_value": {"$eq": 1}
            },
        )
        ...
        self.db.collection.insert_one(
            {"partialFilterExpression_value": 3, "value": 4}
        )
        self.db.collection.insert_one(
            {"partialFilterExpression_value": 3, "value": 4}
        )
        ...
        self.db.collection.insert_one(
            {"partialFilterExpression_value": 1, "value": 2}
        )
        self.db.collection.insert_one(
    ...
    def test_sparse_unique_index(self):
        self.db.collection.create_index(
            [("value", 1)], unique=True, sparse=True
        )
    ...
    def test__create_idx_information(self):
        ...
        self.assertEqual(
            {"_id_"}, set(self.db.collection.index_information().keys())
        )
    ...
    def test__filter_unknown_top_level(self):
        ...
        self.assertEqual(
            "unknown top level operator: $ne", str(error.exception)
        )
    ...
    def test__find_or(self):
        ...
        self.assertEqual(
            ...
            [
                d["x"]
                for d in self.db.collection.find({"$or": [{"x": 4}, {"x": 2}]})
            ],
        )
    ...
    def test__find_projection_with_subdoc_lists_refinements(self):
        ...
        with self.assertRaises(mongomock.OperationFailure):
            self.db.collection.find_one(
                {"a": 1},
                collections.OrderedDict([("a", 1), ("b.c", 1), ("b", 1)]),
            )
    ...
    def test__with_options(self):
        ...
        self.db.collection.with_options(
            write_concern=self.db.collection.write_concern
        )
        self.db.collection.with_options(write_concern=WriteConcern(w=1))
        self.db.collection.with_options(
            read_concern=self.db.collection.read_concern
        )
        self.db.collection.with_options(
    ...
    def test__with_options_different_write_concern(self):
        ...
        self.assertNotEqual(
            self.db.collection.write_concern, col2.write_concern
        )
    ...
    def test__with_options_different_read_preference(self):
        ...
        self.assertEqual(
            "primary", self.db.collection.read_preference.mongos_mode
        )
        self.assertNotEqual(
            self.db.collection.read_preference, col2.read_preference
        )
    ...
    def test__codec_options(self):
        self.assertEqual(
            codec_options.CodecOptions(), self.db.collection.codec_options
        )
        self.db.collection.with_options(codec_options.CodecOptions())

    def test__codec_options_without_pymongo(self):
        self.assertEqual(
            self.db.collection.codec_options, self.db.codec_options
        )
    ...
    def test__with_options_not_implemented(self):
        ...
        with self.assertRaises(NotImplementedError):
            self.db.collection.with_options(
                codec_options=_CodecOptions(None, True, 3)
            )
    ...
    def test__update_current_date(self):
        for type_specification in [True, {"$type": "date"}]:
            self.db.collection.update_one(
                {},
                {"$currentDate": {"updated_at": type_specification}},
                upsert=True,
            )
    ...
    def test_datetime_precision(self):
        ...
        self.db.collection.update_one(
            ...
            {
                "$set": {
                    "new_date_too_precise": too_precise_dt,
                    "new_date": mongo_dt,
                }
            },
            upsert=True,
    ...
    def test__configure_client_tz_aware(self):
        for tz_awarness in (True, False):
            ...
            if tz_awarness:
                ...
                self.assertEqual(
                    timedelta(0), returned.tzinfo.utcoffset(returned)
                )
    ...
    def test__cursor_sort(self):
        ...
        self.assertEqual(
            [1, 2, 3], [doc["a"] for doc in coll.find().sort("a")]
        )
        self.assertEqual(
            [3, 2, 1], [doc["a"] for doc in coll.find().sort("a", -1)]
        )
    ...
    def test__cursor_sort_composed(self):
        ...
        self.assertEqual(
            [2, 3, 1],
            [doc["_id"] for doc in coll.find().sort((("b", 1), ("a", 1)))],
        )
    ...
    def test__cursor_sort_projection(self):
        ...
        self.assertEqual(
            [1, 2, 3],
            [doc["b"] for doc in col.find(projection=["b"]).sort("a")],
        )
    ...
    def test__cursor_sort_dicts(self):
        ...
        self.assertEqual(
            [1, 3, 2], [doc["_id"] for doc in col.find().sort("b")]
        )
    ...
    def test__bulk_write_update_one(self):
        ...
        # Upsert == True
        operations = [
            pymongo.UpdateOne({"a": 1}, {"$set": {"a": 3}}, upsert=True)
        ]
        result = self.db.collection.bulk_write(operations)
    ...
    def test__bulk_write_update_many(self):
        ...
        # Upsert == True
        operations = [
            pymongo.UpdateMany({"a": 2}, {"$set": {"a": 3}}, upsert=True)
        ]
        result = self.db.collection.bulk_write(operations)
    ...
    @skipIf(
        helpers.PYMONGO_VERSION >= version.parse("4.0"), "pymongo v4 or above"
    )
    def test__bulk_write_update_id(self):
        ...
        self.assertEqual(
            ...
            [
                e["errmsg"]
                for e in err_context.exception.details["writeErrors"]
            ],
        )
    ...
    def test__find_or_and(self):
        ...
        self.assertEqual(
            [2], [d["x"] for d in self.db.collection.find(search_filter)]
        )
    ...
    def test__aggregate_replace_root_static(self):
        ...
        self.assertListEqual(
            [{"document": "new"}, {"document": "new"}], list(actual)
        )
    ...
    def test__aggregate_lookup_missing_operator(self):
        ...
        self.assertEqual(
            "Must specify 'from' field for a $lookup", str(err.exception)
        )
    ...
    def test__aggregate_lookup_operator_not_string(self):
        ...
        self.assertEqual(
            "Arguments to $lookup must be strings", str(err.exception)
        )
    ...
    def test__aggregate_graph_lookup_dot_in_as_field(self):
        ...
        self.assertIn(
            "Although '.' is valid in the 'as' parameter", str(err.exception)
        )
    ...
    def test__aggregate_project_if_null(self):
        ...
        self.assertEqual(
            [{"a": "<present_a>", "b": "<missing_b>"}], list(actual)
        )
            ...
            [
                {
                    "$project": {
                        ...
                        "split_string_missing": {
                            "$split": ["$missingField", "l"]
                        },
                        "split_delimiter_none": {"$split": ["$a", None]},
                        "split_delimiter_missing": {
                            "$split": ["$a", "$missingField"]
                        },
                        "split": {"$split": ["$a", "l"]},
            ...
            [
                {
                    "$project": {
                        "Hello": {
                            "$regexMatch": {"input": "$a", "regex": "Hel*o"}
                        },
                        "Word": {
                            "$regexMatch": {"input": "$b", "regex": "Word"}
                        },
                        "missing-field": {
    ...
    def test__filter_ne_multiple_keys(self):
        ...
        # $ne
        results = self.db.collection.find(
            {"cases.total": {"$gt": 1, "$ne": 3}}
        )
        self.assertEqual([2], [doc["_id"] for doc in results])

        # $nin
        results = self.db.collection.find(
            {"cases.total": {"$gt": 1, "$nin": [1, 3]}}
        )
        self.assertEqual([2], [doc["_id"] for doc in results])
    ...
    def test_insert_many_bulk_write_error_details(self):
        ...
        self.assertEqual(
            [11000], [error.get("code") for error in write_errors]
        )
    ...
    def test_insert_bson_validation(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "cannot encode object: {'b'}, of type: <class 'set'>",
        )
    ...
    def test_insert_bson_invalid_encode_type(self):
        ...
        self.assertEqual(
            str(cm.exception), "key '$foo' must not start with '$'"
        )
            ...
            [
                ...
                {"$unwind": "$a.a"},
                {
                    "$group": {
                        ...
                        "update_time": {"$first": "$update_time"},
                        "a": {
                            "$push": {"b": "$a.a.b", "weight": "$a.a.weight"}
                        },
                    }
                },
                {"$out": "ooo"},
    ...
    def test__aggregate_group_sum_for_decimal(self):
        ...
        collection.insert_one(
            {"_id": 1, "a": 2, "b": 3, "c": "$d", "d": decimal_value}
        )
        actual = collection.aggregate(
            [
                {
    ...
    def test__aggregate_subtract_milliseconds_from_date(self):
        ...
        self.assertEqual(
            [datetime(2014, 7, 4, 13, 0, 3, 20000)],
            [d["since"] for d in actual],
        )
    ...
    def test__write_concern(self):
        ...
        collection = self.db.get_collection(
            "a",
            write_concern=WriteConcern(w=2, wtimeout=100, j=True, fsync=False),
        )
        self.assertEqual(
            {
                "fsync": False,
    ...
    def test__read_preference_default(self):
        ...
        self.assertEqual(
            0, self.db.collection.read_preference.min_wire_version
        )
    ...
    def test__read_preference(self):
        collection = self.db.get_collection(
            "a", read_preference=ReadPreference.NEAREST
        )
    ...
    def test__bulk_write_unordered(self):
        ...
        self.assertCountEqual(
            [1, 2, 3], [d["_id"] for d in self.db.collection.find()]
        )
        self.assertEqual(3, err_context.exception.details["nInserted"])
        self.assertEqual(
            [2, 4],
            [e["index"] for e in err_context.exception.details["writeErrors"]],
        )
    ...
    def test__bulk_write_ordered(self):
        ...
        self.assertCountEqual(
            [1, 2], [d["_id"] for d in self.db.collection.find()]
        )
    ...
    def test__insert_do_not_modify_input(self):
        ...
        self.assertEqual(
            dict(original_document, date=None),
            dict(stored_document, date=None),
        )
        self.assertNotEqual(
            original_document,
            stored_document,
        ...
        self.assertNotEqual(
            original_document["date"].timestamp(),
            stored_document["date"].timestamp(),
        )
        self.assertEqual(
            ...
            [
                {
                    "$addFields": {
                        ...
                        "str_negative_number": {
                            "$toDecimal": "$str_negative_number"
                        },
                        "str_decimal_number": {
                            "$toDecimal": "$str_decimal_number"
                        },
    ...
    def test_list_collection_names_filter(self):
        ...
        # test equal
        assert col_name in self.db.list_collection_names(
    ...
    def test__equality(self):
        ...
        self.assertEqual(
            ...
            mongomock.MongoClient("localhost").db.collection,
        )
        self.assertNotEqual(
            client.db.collection,
            mongomock.MongoClient("example.com").db.collection,
        )
    ...
    @skipUnless(
        helpers.PYMONGO_VERSION
        and helpers.PYMONGO_VERSION < version.parse("3.12"),
    ...
    def test__bad_type_as_a_read_concern_returns_type_error(self):
        with self.assertRaises(
            ...
            mongomock.collection.Collection(
                self.db, "foo", None, read_concern="bar"
            )
