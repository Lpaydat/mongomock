...
class MultiCollection(object):
    def __init__(self, conns):
        ...
        self.compare_exceptions = Foreach(
            self.conns, compare=_COMPARE_EXCEPTIONS
        )


class Foreach(object):
    def __init__(
        self, objs, compare, ignore_order=False, method_result_decorators=()
    ):
        self.___objs = objs
...
class ForeachMethod(object):
    def __init__(
        self, objs, compare, ignore_order, method_name, decorators, sort_by
    ):
        self.___objs = objs
        self.___compare = compare
        self.___ignore_order = ignore_order
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.___compare:
            _assert_no_diff(
                results,
                ignore_order=self.___ignore_order,
                sort_by=self.___sort_by,
            )
        return results
...
def _assert_no_diff(results, ignore_order, sort_by):
    if _result_is_cursor(results) or _result_is_command_cursor(results):
        ...
        if sort_by is not None:
            ...
            value_processor = functools.partial(
                _expand_cursor, sort=ignore_order
            )
    else:
...
def _result_is_cursor(results):
    return any(
        type(result).__name__ == "Cursor" for result in results.values()
    )


def _result_is_command_cursor(results):
    return any(
        type(result).__name__ == "CommandCursor" for result in results.values()
    )
